...
def _render_items(items: list[SourceItem]) -> str:
    lines = []
    max_text_chars = config.SOURCE_TEXT_CHARS
    if "github_models" in _provider_chain():
        max_text_chars = min(max_text_chars, 2600)
    for it in items:
...
def extract_pain_points(items: list[SourceItem], batch_size: int | None = None) -> list[PainPoint]:
...
def _source_links_for_ids(
    ...
    for sid in source_ids:
        ...
        if url and url not in seen:
            seen.add(url)
            srcs.append({"site": _SITE.get(id_to_source.get(sid, ""), "Source"), "url": url})
    return _source_markdown(srcs[:limit])


def write_issue_brief(
    ...
    """Cheap daily output: new non-duplicate issues, no extra LLM writing call."""
    id_to_url = id_to_url or {}
    id_to_source = id_to_source or {}
    if not leads:
        return (
            f"# Problembrief — {date_str}\n\n"
            "## TL;DR\n\n"
            f"Scanned {item_count} items. No new non-duplicate issues passed the bar today.\n"
        )
    ...
    lines = [
        ...
        "",
        (
            f"Scanned {item_count} items and found {len(leads)} new, non-duplicate "
            f"issues. Strongest signal: **{strongest.summary}**"
        ),
        "",
        "## New Issues",
        "",
    ...
    for lead in leads:
        title = lead.summary.rstrip(".")
        sources_md = _source_links_for_ids(lead.source_ids, id_to_url, id_to_source)
        lines.extend(
            [
                f"### {title}",
                "",
                f"**Category:** {(lead.category or '').replace('_', ' ').title()}",
                f"**Problem:** {lead.summary}",
                f"**Evidence:** {lead.evidence or 'No short evidence captured.'}",
                f"**Current workaround:** {lead.current_workaround or 'Unclear from the source.'}",
                (
                    "**Manual steps:** "
                    + (", ".join(lead.manual_steps) if lead.manual_steps else "Unclear from the source.")
                ),
                f"**Who feels it:** {lead.user_role or 'Unclear from the source.'}",
                f"**Likely buyer:** {lead.buyer_role or 'Unclear from the source.'}",
                (
                    f"**Scores:** pain {lead.pain} · freq {lead.frequency} · "
                    f"build {lead.buildability} · market {lead.market_signal} · "
                    f"interest {lead.personal_interest}"
                ),
                f"**Sources:** {sources_md}" if sources_md else "",
                "",
            ]
        )
    lines.append("Generated by Problembrief.")
